...
int main()
{
	...
	for (int i = 0; i < m; i++)
	{
		...
		if (strcmp(operacao, "UPDATE") == 0)
		{
			...
			if (strcmp(sigla, "BRML3") == 0)
			{
				printf("estou mexendo na BRML3\n");
			}
			...
			else
			{
				...
				*novoValorKey = valor;
				...
				if (strcmp(sigla, "BRML3") == 0)
				{
					printf("vou buscar pra ver se ela existe BRML3\n");

					if (hash_table_get(hash, "BRML3") == NULL)
					{
						printf("HASH: nao existe BRML3\n");
					}
					else
					{
						printf("HASH: existe BRML3\n");
					}

					Empresa *empresa_busca = hash_table_get(hash, "BRML3");
					float valor = empresa_busca->valorUnitario;
					printf("valorUnitario: %.2f\n", valor);
					if (binary_tree_get(tree, &valor) == NULL)
					{
						printf("TREE: nao existe BRML3\n");
					}
					else
					{
						printf("TREE: existe BRML3\n");
					}
				}
